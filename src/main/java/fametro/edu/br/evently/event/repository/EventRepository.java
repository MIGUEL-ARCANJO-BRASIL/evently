...
public interface EventRepository extends JpaRepository<Event, UUID> {
    ...
    @Query("SELECT e FROM Event e WHERE e.eventStatus = :status " +
            ...
            "AND (CAST(:category AS String) IS NULL OR e.category.name = CAST(:category AS String)) " +
            "AND (CAST(:query AS String) IS NULL OR LOWER(e.title) LIKE LOWER(CONCAT('%', CAST(:query AS String), '%'))) " +
